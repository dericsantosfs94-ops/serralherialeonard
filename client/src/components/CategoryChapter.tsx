...
export function CategoryChapter({
  ...
  const bg = tone === "emerald" ? "bg-[var(--paper-green)]" : "bg-[var(--background)]";
  ...
  return (
    <section id={anchor} className={`relative py-24 md:py-32 ${bg} overflow-hidden`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-end mb-14">
            <div className={`lg:col-span-7 ${reversed ? "lg:order-2" : ""}`}>
              ...
              <h2 className="mt-4 font-display text-[2.2rem] leading-[1.1] sm:text-[3.1rem] font-semibold text-[var(--foreground)]">
                {title}
                {titleAccent && (
                  <span className="text-[var(--emerald-deep)]">{titleAccent}</span>
                )}
              </h2>
            </div>
            <div className={`lg:col-span-5 ${reversed ? "lg:order-1" : ""}`}>
              <p className="text-[1rem] leading-relaxed text-[oklch(0.4_0.018_150)] font-light">{description}</p>
              <a
                href={cta}
                target="_blank"
          ...
          <FadeUp className={`md:col-span-7 ${reversed ? "md:order-2" : ""}`}>
            <button onClick={() => setActive(big)} className="group block w-full text-left" aria-label={`Ver detalhes do projeto ${big.title}`}>
              ...
              <div className="mt-3.5 pr-1">
                ...
                <h3 className="mt-1.5 font-display text-[1.5rem] md:text-[1.75rem] font-semibold text-[var(--foreground)] group-hover:text-[var(--emerald-deep)] transition-colors">{big.title}</h3>
                <p className="mt-1 text-[0.85rem] text-[oklch(0.36_0.018_150)] font-light">{big.description}</p>
              </div>
            </button>
          </FadeUp>
      ...
      {active && (
        ...
        >
          <div
            className="relative max-w-4xl w-full max-h-[90svh] bg-[var(--background)] border border-[var(--emerald-deep)]/25% rounded-sm overflow-y-auto grid md:grid-cols-[1.4fr_1fr]"
