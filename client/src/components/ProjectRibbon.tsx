...
import { companyData, type PortfolioItem } from "../data/company";

const ribbonItems: PortfolioItem[] = companyData.portfolio.slice(0, 8);

function RibbonCard({ item }: { item: PortfolioItem }) {
  const target = item.category === "portoes" ? "#portoes" : item.category === "coberturas" ? "#coberturas" : "#vidracaria";
